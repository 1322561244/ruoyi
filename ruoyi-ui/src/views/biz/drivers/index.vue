...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/biz/drivers/importData", // todo
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    /* 导入按钮操作 */
    handleImport() {
      this.upload.title = "司机信息导入"; // todo
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download(
        "/biz/drivers/importTemplate",
        {},
        `drivers_base_template_${new Date().getTime()}.xlsx`
      ); // todo
    },
    // 文件上传中处理
