  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="班次id" prop="shiftsId">
          <treeselect v-model="form.shiftsId" :options="dateOptions" :normalizer="normalizer" placeholder="请选择班次id" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    getList() {
      ...
      listDate(this.queryParams).then(response => {
        this.dateList = this.handleTree(response.data, "shiftsId", "shiftsId");
        this.loading = false;
      });
    },
    /** 转换班次日期管理数据结构 */
    ...
    getTreeselect() {
      listDate().then(response => {
        this.dateOptions = [];
        const data = { shiftsId: 0, shiftsId: '顶级节点', children: [] };
        data.children = this.handleTree(response.data, "shiftsId", "shiftsId");
    ...
    handleAdd(row) {
      this.reset();
      this.getTreeselect();
      if (row != null && row.shiftsId) {
        this.form.shiftsId = row.shiftsId;
      } else {
        this.form.shiftsId = 0;
      }
      this.open = true;
      this.title = "添加班次日期管理";
    },
    ...
    handleUpdate(row) {
      ...
      getDate(row.dateId).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除班次日期管理编号为"' + row.dateId + '"的数据项？').then(function() {
        return delDate(row.dateId);
      }).then(() => {
        this.getList();
